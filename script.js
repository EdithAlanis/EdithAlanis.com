...
function renderPatents(){
  const grid=$("patentGrid"); if(!grid) return;
  grid.innerHTML = patents.map(p=>`
    <article class="patent-card">
      <h3>${p.kind} ${p.number}</h3>
      <p class="meta">${p.status} · ${p.location}</p>
      <p><strong>Inventores</strong></p>
      <ul>${p.inventors.map(i=>`<li>${i}</li>`).join("")}</ul>
    </article>`).join("");
}
...
function renderNetwork(query=""){
  ...
  if($("coinventorCount")) $("coinventorCount").textContent = `${list.length} registros`;
  ...
  const stateList=$("stateList"); if(!stateList) return;
  ...
  if($("stateTitle")) $("stateTitle").textContent = activeState || "Coinventores";
  ...
  const tabs=$("municipalityTabs");
  tabs.innerHTML = `<button class="${activeCity==='all'?'active':''}" data-city="all">Todos (${statePeople.length})</button>` + cities.map(c=>`<button class="${activeCity===c?'active':''}" data-city="${c}">${c} (${statePeople.filter(p=>p.city===c).length})</button>`).join("");
  tabs.querySelectorAll("button").forEach(btn=>btn.onclick=()=>{activeCity=btn.dataset.city; renderNetwork($("coinventorSearch").value);});
...
function renderPortfolio(){
  ...
  if($("portfolioCount")) $("portfolioCount").textContent = `${list.length} registros`;
  const grid=$("portfolioGrid"); if(!grid) return;
  grid.innerHTML = list.map(p=>`<article class="portfolio-card"><h3>${p.group==="presentada" ? "Área" : "Registro"} ${p.id}</h3><p><strong>Área o tipo</strong><br>${p.area}</p><span class="badge">${p.badge} ${p.status}</span></article>`).join("");
}
